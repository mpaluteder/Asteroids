import { canvasWidth, canvasHeight } from './utility/Canvas';
  ...
  static createFromRandomEdge(
    ...
    const randomPosition = new Vec2(
      lockXPlane
        ? lockReverse
          ? addedSpacing
          : canvasWidth - addedSpacing
        : Math.max(
            Math.floor(Math.random() * canvasWidth - addedSpacing),
            addedSpacing
          ),
      !lockXPlane
        ? lockReverse
          ? addedSpacing
          : canvasHeight - addedSpacing
        : Math.max(
            Math.floor(Math.random() * canvasHeight - addedSpacing),
            addedSpacing
          )
    );
